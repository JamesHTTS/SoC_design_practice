...
module top_mux_dec_reg (
 ...
 logic [7:0] mux_low,mux_high;
//1. decoder
decoder2to4 u_dec(.in(waddr),.en(we),.out(we_vec));
//2. register
register8 u_reg0(.clk,.rst_n,.we(we_vec[0]),.d(wdata),.q(reg0_q));
register8 u_reg1(.clk,.rst_n,.we(we_vec[1]),.d(wdata),.q(reg1_q));
register8 u_reg2(.clk,.rst_n,.we(we_vec[2]),.d(wdata),.q(reg2_q));
register8 u_reg3(.clk,.rst_n,.we(we_vec[3]),.d(wdata),.q(reg3_q));
//3.Mux
