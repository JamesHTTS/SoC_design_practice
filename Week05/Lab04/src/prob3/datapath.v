...
wire [7:0] alu_b;
    ...
    //   );
reg_file u_rf (.clk(clk),.rst_n(rst_n),.raddr1(raddr1),.raddr2(raddr2),.we(we),.waddr(waddr),.rdata1(rdata1),.rdata2(rdata2),
.wdata(alu_result));

mux2to1_8bit u_mux_b ( .a(rdata2),.b(imm),.sel(src_b_sel),.y(alu_b));

alu_unit u_alu (.a(rdata1),.b(alu_b),.op(alu_op),.y(alu_result),.zero(zero));
